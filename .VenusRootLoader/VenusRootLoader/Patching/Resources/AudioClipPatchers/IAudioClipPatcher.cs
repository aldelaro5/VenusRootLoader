...
/// <summary>
/// A patcher that handles patching specific <see cref="AudioClip"/> individually given that the resources path
...
internal interface IAudioClipPatcher
{
    ...
    /// <summary>
    /// Patches the original <see cref="AudioClip"/> given that the game requested to load it using the resources path
    ...
    /// <param name="original">The original <see cref="AudioClip"/> that would be returned if the patcher wasn't present.</param>
    /// <returns>The patched <see cref="AudioClip"/> asset.</returns>
